...
class TrafficResnet:
  ...
  def Setup():
    ...
    TrafficResnet._label_map = text_format.Parse(s, mymap)
  ...
  def get_label(self, index):
    return TrafficResnet._label_map.item[index].display_name
  ...
  def Apply(self, batched_data_dict, batch_size, istub):
    if (batch_size != len(batched_data_dict["original_shape"])):
  ...
  def GetBatchedResultArray(self, batched_result_dict, batch_size):
    if (batch_size != len(batched_result_dict["original_shape"])):
