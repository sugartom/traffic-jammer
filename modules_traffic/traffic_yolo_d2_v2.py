    ...
    if (batch_size != len(batched_result_dict[batched_result_dict.keys()[0]])):
      ...
      for i in range(batch_size):
        ...
        for j in range(1):
          ...
          output.append("%s|%s|%s|%s|%s|%s" % (str(d['topleft']['x']), str(d['topleft']['y']), str(d['bottomright']['x']), str(d['bottomright']['y']), str(d['confidence']), str(d['label'])))
        # output = output[:-1]
        my_dict["objdet_output"] = output
        my_dict["raw_image"] = [batched_result_dict["raw_image"][i]] * len(output)
        my_dict["output_flag"] = [1] + [0] * (len(output) - 1)
        batched_result_array.append(my_dict)
  ...
  def GetResultList(self, result_dict):
    ...
    for i in range(len(result_dict[result_dict.keys()[0]])):
      if (result_dict["objdet_output"][i] != ""):
        result_list.append({"objdet_output": result_dict["objdet_output"][i], "raw_image": result_dict["raw_image"][i], "output_flag": result_dict["output_flag"][i]})
    return result_list
